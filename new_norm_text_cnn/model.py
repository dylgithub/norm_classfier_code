...
import os
from utils import weight_layers
...
class ELMOClassifierModel(object):
    def __init__(self,n_dims,max_chars_per_word,graph,options_file,lm_weights_file,max_batch_size=128):
        self.n_dims=n_dims*2
        self.max_chars_per_word=max_chars_per_word
        self.graph=graph
        self.options_file=options_file
        self.lm_weights_file=lm_weights_file
        self.max_batch_size=max_batch_size
        #为什么这样设定维度，是通过字符使用的方法
        self.inputs=tf.placeholder(shape=[None,None,self.max_chars_per_word],name='elmo_inputs',dtype=tf.int32)
        self.targets=tf.placeholder(shape=[None],name='labels',dtype=tf.int64)
        self.is_training=tf.placeholder(shape=(),name='is_training',dtype=tf.bool)
    ...
    def _add_variable(self):
        #因为使用的是字符 级别的具体模型构建看BidirectionalLanguageModel下的注释有介绍
        self.bilm=BidirectionalLanguageModel(self.options_file,self.lm_weights_file,
                                             max_batch_size=self.max_batch_size)
        self.attention_context_vector=tf.get_variable(shape=[self.n_dims],name='context_vector')
        self.global_step=tf.Variable(0,name='global_step',trainable=False)
    def _add_predict_op(self):
        elmo_ops=self.bilm(self.inputs)
        elmo_inputs=weight_layers('elmo',elmo_ops,l2_coef=0.0)

        attention_vector=tf.reduce_sum(tf.multiply(elmo_inputs['weighted_op'],self.attention_context_vector),axis=2,keepdims=True)
        attention_weights=tf.nn.softmax(attention_vector,axis=1)
        attention_outputs=tf.reduce_sum(tf.multiply(elmo_inputs['weighted_op'],attention_weights),axis=1)

        #
        W=tf.get_variable('weights',shape=[self.n_dims,2],initializer=layers.variance_scaling_initializer(),dtype=tf.float32)
        b=tf.get_variable('bias',shape=[2],initializer=tf.constant_initializer(0.0))
        self.logits=tf.matmul(attention_outputs,W)+b
        self.probs=tf.nn.softmax(self.logits,axis=-1)
        self.predicts=tf.argmax(self.probs,axis=-1,name='predict')
    def _add_loss_op(self):
        cross_entropy=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.targets,logits=self.logits)
    ...
    def _add_train_op(self):
        ...
        self.train_op=optimizer.apply_gradients(zip(grads,tvars),name='train_op',global_step=self.global_step)
    def _add_accuracy_op(self):
        labels=tf.cast(self.targets,tf.float32)
        predict=tf.cast(self.predicts,tf.float32)
    def _add_summary_op(self):
        self.summary_op=tf.summary.merge_all()
    def _add_saver(self):
        self.saver=tf.train.Saver(tf.global_variables())
    def get_feed_dict(self,x,y=None,is_training=True):
        feed_dict={
            self.inputs:x,
